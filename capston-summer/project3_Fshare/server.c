    ...
    while(one = readdir(src)){
        ...
        if(one->d_type == DT_DIR){ //dir: recursively explore 
            ...
            if(listFiles(nextSrc, conn)) { 
                ...
                if(send(conn, &failure, 4, 0) <= 0){
                    perror("[cannot send is_error]");
                }
                closedir(src);
                return EXIT_FAILURE;
            }
            ...
            //send payload 
            if(!send(conn, &nextSrc[strlen(homePath)+1], sheader.payload_size, 0)){
...
int showFile(struct client_header cheader, int conn){
        ...
        if(!(s = recv(conn, buf, cheader.src_path_len, 0))){
